...
class fc_email_nc extends _modelo_parent_sin_codigo
{
    ...
    public function alta_bd(array $keys_integra_ds = array('descripcion')): array|stdClass
    {
        ...
        if($existe){
            $r_fc_email_nc = $this->filtro_and(filtro: $filtro);
            if(errores::$error){
                return $this->error->error(mensaje: 'Error al obtener registro',data:  $r_fc_email_nc);
            }
            if($r_fc_email_nc->n_registros === 0){
                return $this->error->error(mensaje: 'Error no existe registro',data:  $r_fc_email_nc);
            }
            if($r_fc_email_nc->n_registros > 1){
                return $this->error->error(mensaje: 'Error existe mas de un registro',data:  $r_fc_email_nc);
            }

            $this->registro_id = $r_fc_email_nc->registros[0]['fc_email_nc_id'];
            ...
            $r_alta_bd->registro = $r_fc_email_nc->registros[0];
            $r_alta_bd->registro_obj = (object)$r_fc_email_nc->registros[0];
    ...
    final public function status(string $campo, int $registro_id): array|stdClass
    {
        ...
        $fc_email_nc = $this->registro(registro_id: $registro_id, retorno_obj: true);
        if(errores::$error){
            return $this->error->error(mensaje: 'Error al obtener fc_email_nc',data:  $fc_email_nc);
        }
        $com_email_cte['status'] = $fc_email_nc->fc_email_nc_status;
        $r_com_email_cte = (new com_email_cte(link: $this->link))->modifica_bd(registro: $com_email_cte,
            id: $fc_email_nc->com_email_cte_id);
        if(errores::$error){

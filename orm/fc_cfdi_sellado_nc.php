...
class fc_cfdi_sellado_nc extends _modelo_parent
{
    public function __construct(PDO $link)
    {
        $tabla = 'fc_cfdi_sellado_nc';
    ...
    public function alta_bd(array $keys_integra_ds = array('codigo', 'descripcion')): array|stdClass
    {
        ...
        $permite_transaccion = (new fc_factura(link: $this->link))->verifica_permite_transaccion(
            modelo_etapa: $modelo_etapa, registro_id: $this->registro['fc_factura_id']);
        if (errores::$error) {
    ...
    {
        $fc_cfdi_sellado_nc = $this->registro(registro_id: $id, retorno_obj: true);
        if (errores::$error) {
            return $this->error->error(mensaje: 'Error al obtener registro', data: $fc_cfdi_sellado_nc);
        }

        $modelo_etapa = new fc_factura_etapa(link: $this->link);
        $permite_transaccion = (new fc_factura(link: $this->link))->verifica_permite_transaccion(
            modelo_etapa: $modelo_etapa, registro_id: $fc_cfdi_sellado_nc->fc_factura_id);
        if (errores::$error) {
            return $this->error->error(mensaje: 'Error verificar transaccion', data: $permite_transaccion);
        }
        errores::$error = true;
    ...
    {
        $fc_cfdi_sellado_nc = $this->registro(registro_id: $registro_id, retorno_obj: true);
        if (errores::$error) {
            return $this->error->error(mensaje: 'Error al obtener registro', data: $fc_cfdi_sellado_nc);
        }
        $modelo_etapa = new fc_factura_etapa(link: $this->link);

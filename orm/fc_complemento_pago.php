...
class fc_complemento_pago extends _transacciones_fc
{
    ...
    {
        $data = parent::data_factura($row_entidad); // TODO: Change the autogenerated stub
        if(errores::$error){
        ...
        foreach ($fc_pagos as $indice_fc_pago=>$fc_pago) {
            ...
            foreach ($fc_pago_pagos as $indice_fc_pago_pago=>$fc_pago_pago) {
                ...
                $Complemento[$indice_fc_pago]->Pagos20->Pago[$indice_fc_pago_pago]->TipoCambioP = $fc_pago_pago['cat_sat_com_tipo_cambio_monto'];
                ...
                foreach ($fc_impuestos_p as $fc_impuesto_p) {
                    ...
                    $fc_traslados_p = $this->fc_traslados_p(fc_impuesto_p_id: $fc_impuesto_p['fc_impuesto_p_id']);
                    if (errores::$error) {
                    ...
                    foreach ($fc_traslados_p as $fc_traslado_p) {
                        ...
                        $fc_traslados_p_part = $this->fc_traslados_p_part(fc_traslado_p_id: $fc_traslado_p['fc_traslado_p_id']);
                        if (errores::$error) {
     ...
     */
    PUBLIC function integra_fc_traslado_p_part(array $fc_traslado_p_part): stdClass|array
    {

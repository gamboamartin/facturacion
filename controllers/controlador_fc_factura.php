...
class controlador_fc_factura extends _base_system_fc
{
    ...
    public function cancela_bd(bool $header, bool $ws = false): array|stdClass
    {
        ...
        if ((new generales())->aplica_relacion_layout_factura) {
            ...
            $rs = $fc_layout_factura_modelo->elimina_con_factura_id(fc_factura_id: $fc_factura_id);
            if (errores::$error) {
                $this->link->rollback();
                return $this->retorno_error(
                    mensaje: 'Error al eliminar la relaciones de la factura',
                    data: $rs,
                    header: $header,
                    ws: $ws
                );

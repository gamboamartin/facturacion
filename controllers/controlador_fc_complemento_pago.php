...
class controlador_fc_complemento_pago extends _base_system_fc {
    ...
    public function genera_pdf(bool $header, bool $ws = false, bool $descarga = true, bool $guarda = false)
    {
        ...
        ob_start();
        include __DIR__ . '/templates/reporte.php';
        $html = ob_get_clean();
        ...
        $dompdf->stream($datos['folio'].'.pdf', ['Attachment' => true]);
    }
    ...
    public function timbra_xml(bool $header, bool $ws = false): array|stdClass
    {
        ...
        $timbrada = $this->modelo_sello->existe(
            filtro: array('fc_cfdi_sellado_cp.fc_complemento_pago_id' => $this->registro_id)
        );
        if (errores::$error) {
            return $this->retorno_error(mensaje: 'Error al validar si el complemento está timbrado',
                data: $timbrada, header: $header, ws: $ws);
        }
        if ($timbrada) {
            return $this->retorno_error(mensaje: 'Error: el complemento de pago ya ha sido timbrado',
                data: $timbrada, header: $header, ws: $ws);
        }

        $r_timbra = parent::timbra_xml(header: $header, ws: $ws);
        if(errores::$error){

...
class _reporte_anual_v2{
    ...
    private function recorre_registros(array $registros,Worksheet $sheet): void
    {
        ...
        foreach ($registros as $registro) {
            ...
            $total = (double)$registro['monto_relacionado'];

            $monto_dispersion = $total / ((1 + $porcentaje_comision) * 1.16);
            $iva = $total * (0.16 / 1.16);
            ...
            $sheet->setCellValue("H{$fila}", $registro['producto']);
            $sheet->setCellValue("I{$fila}", $registro['numero_empleados']); // NUMERO DE EMPLEADOS
            ...
            $sheet->setCellValue("O{$fila}", $iva);
            $sheet->setCellValue("P{$fila}", $registro['monto_relacionado']);
            $sheet->setCellValue("Q{$fila}", $registro['total']);
    ...
    private function procesa_registros_fmt3(array $registros, string $mes): void
    {
        ...
        foreach ($registros as $registro) {
            ...
            $total = (float)$registro['monto_relacionado'];
    ...
    private function headers_fmt1_fmt2(): array
    {
        return [
            ...
            'G1' => 'PERIODO',
            'H1' => 'PRODUCTO',
            'I1' => 'NUMERO DE EMPLEADOS',
            'J1' => 'FECHA DE OPERACIÓN',
            'K1' => 'FAC.',
            'L1' => 'LAYOUT',
            'M1' => 'COMISION CLIENTE',
            'N1' => 'MONTO DE DISPERSION',
            'O1' => 'IVA',
            'P1' => 'TOTAL',
            'Q1' => 'FACTURA TOTAL',
        ];
    ...
    private function fmt1_fmt2_styles(Worksheet $sheet, int $ultimaFila): void
    {
        ...
        $sheet->getStyle("K2:K{$ultimaFila}")->getAlignment()
            ->setHorizontal(Alignment::HORIZONTAL_CENTER);

        $sheet->getStyle("I2:I{$ultimaFila}")->getAlignment()
            ->setHorizontal(Alignment::HORIZONTAL_CENTER);

        $sheet->getStyle("L2:L{$ultimaFila}")->getAlignment()
            ->setHorizontal(Alignment::HORIZONTAL_CENTER);
    ...
    private function base_query(): string
    {
        return "SELECT
                        ...
                        fc_factura.folio AS numero_factura,
                        fc_layout_nom.id AS layout_id,
                        fc_layout_nom.fecha_pago AS fecha_operacion,
                        producto.descripcion AS producto,
                        fc_factura.porcentaje_comision_cliente AS porcentaje_comision,
                        fc_layout_factura.monto_relacionado AS monto_relacionado,
                        fc_factura.total
                    FROM
                        fc_layout_factura
                        LEFT JOIN fc_factura ON fc_layout_factura.fc_factura_id = fc_factura.id 

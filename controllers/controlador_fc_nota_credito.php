...
class controlador_fc_nota_credito extends _base_system_fc {
    ...
    public function alta_partida_bd(bool $header, bool $ws = false){
        ...
        $nota_credito = (new fc_nota_credito($this->link))->get_factura(fc_nota_credito_id: $this->registro_id);
        if(errores::$error){
            return $this->retorno_error(mensaje: 'Error al obtener factura', data: $nota_credito, header: $header, ws: $ws);
        }

        $registro = $_POST;
        $registro['fc_nota_credito_id'] = $this->registro_id;
    ...
    private function button_elimina_correo(int $fc_email_id, int $fc_nota_credito_id): array|string
    {
        $params = $this->params_button_partida(accion_retorno: 'correo', fc_nota_credito_id: $fc_nota_credito_id);
        if (errores::$error) {
    ...
    private function button_status_correo(int $fc_email_id, int $fc_nota_credito_id): array|string
    {
        $params = $this->params_button_partida(accion_retorno: 'correo', fc_nota_credito_id: $fc_nota_credito_id);
        if (errores::$error) {
            return $this->errores->error(mensaje: 'Error al generar params', data: $params);
    ...
    public function cancela(bool $header, bool $ws = false){
        ...
        $columns_ds = array('fc_nota_credito_folio','com_cliente_rfc','fc_nota_credito_total','fc_nota_credito_fecha');
        $fc_nota_credito_id = $this->html_fc->select_fc_nota_credito_id(cols: 12, con_registros: true,
            id_selected: $this->registro_id, link: $this->link, columns_ds: $columns_ds, disabled: true,
            filtro: $filtro);
        if(errores::$error){
            return $this->retorno_error(mensaje: 'Error al obtener factura',data:  $fc_nota_credito_id, header: $header,ws:$ws);
        }
    ...
    public function cancela_bd(bool $header, bool $ws = false): array|stdClass
    {
        ...
        $r_fc_cancelacion = (new fc_nota_credito(link: $this->link))->cancela_bd(
            cat_sat_motivo_cancelacion_id: $_POST['cat_sat_motivo_cancelacion_id'], fc_nota_credito_id: $this->registro_id);
        if(errores::$error){
    ...
    public function correo(bool $header, bool $ws = false): array|stdClass
    {
        ...
        $fc_nota_credito_folio = (new fc_nota_credito_html(html: $this->html_base))->input_folio(cols: 12,row_upd: $row_upd,
            value_vacio: false, disabled: true);
        if (errores::$error) {
            return $this->errores->error(mensaje: 'Error al maquetar input', data: $fc_nota_credito_folio);
        }

        $this->inputs->fc_nota_credito_folio = $fc_nota_credito_folio;
        ...
        $hidden_row_id = $this->html->hidden(name: 'fc_nota_credito_id',value:  $this->registro_id);
        if (errores::$error) {
        ...
        foreach ($emails_facturas as $indice=>$email_factura){
            ...
            $link_elimina = $this->button_elimina_correo(fc_email_id: $email_factura['fc_email_id'], fc_nota_credito_id: $this->registro_id);
            if (errores::$error) {
                return $this->errores->error(mensaje: 'Error al generar link elimina_bd para partida', data: $link_elimina);
            }
            $emails_facturas[$indice]['elimina_bd'] = $link_elimina;

            $link_status = $this->button_status_correo(fc_email_id: $email_factura['fc_email_id'], fc_nota_credito_id: $this->registro_id);
            if (errores::$error) {
                return $this->errores->error(mensaje: 'Error al generar link elimina_bd para partida', data: $link_elimina);
            }
        ...
        $button_fc_nota_credito_modifica =  $this->html->button_href(accion: 'modifica', etiqueta: 'Ir a Nota Credito',
            registro_id: $this->registro_id,
            seccion: 'fc_nota_credito', style: 'warning', params: array());
        if (errores::$error) {
            return $this->errores->error(mensaje: 'Error al generar link', data: $button_fc_nota_credito_modifica);
        }

        $this->button_fc_nota_credito_modifica = $button_fc_nota_credito_modifica;
        return $this->inputs;
    }
    ...
    public function descarga_xml(bool $header, bool $ws = false){
        $ruta_xml = (new fc_nota_credito_documento(link: $this->link))->get_factura_documento(fc_nota_credito_id: $this->registro_id,
            tipo_documento: "xml_sin_timbrar");
        if(errores::$error){
            return $this->retorno_error(mensaje: 'Error al obtener XML',data:  $ruta_xml, header: $header,ws:$ws);
        }

        $fc_nota_credito = $this->modelo->registro(registro_id: $this->registro_id, retorno_obj: true);
        if(errores::$error){
            return $this->retorno_error(mensaje: 'Error al obtener factura',data:  $fc_nota_credito, header: $header,ws:$ws);
        }

        $file_name = $fc_nota_credito->fc_nota_credito_serie.$fc_nota_credito->fc_nota_credito_folio.'.xml';
    ...
    private function doc_documento_id(stdClass $fc_nota_credito, string $pdf){
        $doc_documento_ins = array();

        $file = $this->file_doc_pdf(fc_nota_credito: $fc_nota_credito,pdf:  $pdf);
        if(errores::$error){
    ...
    public function envia_cfdi(bool $header, bool $ws = false){
        ...
        $inserta_notificacion = (new fc_nota_credito(link: $this->link))->inserta_notificacion(registro_id: $this->registro_id);
        if(errores::$error){
            return $this->retorno_error(mensaje: 'Error al insertar notificacion',data:  $inserta_notificacion, header: $header,ws:$ws);
        }

        $envia_notificacion = (new fc_nota_credito(link: $this->link))->envia_factura(fc_nota_credito_id: $this->registro_id);
        if(errores::$error){
    ...
    public function envia_factura(bool $header, bool $ws = false){
        ...
        $this->link->beginTransaction();
        $notifica = (new fc_nota_credito(link: $this->link))->envia_factura(fc_nota_credito_id: $this->registro_id);
        if(errores::$error){
            $this->link->rollBack();
    ...
    public function exportar_documentos(bool $header, bool $ws = false){
        ...
        $name_zip = $fc_nota_credito->fc_nota_credito_serie.$fc_nota_credito->fc_nota_credito_folio;

        $archivos = array();
        $archivos[$ruta_xml] = "$fc_nota_credito->fc_nota_credito_serie$fc_nota_credito->fc_nota_credito_folio.xml";
        $archivos[$ruta_pdf] = "$fc_nota_credito->fc_nota_credito_serie$fc_nota_credito->fc_nota_credito_folio.pdf";
    ...
    private function fc_nota_credito_documento_ins(int $doc_documento_id, int $fc_nota_credito_id): array
    {
        $fc_nota_credito_documento_ins['fc_nota_credito_id'] = $fc_nota_credito_id;
        $fc_nota_credito_documento_ins['doc_documento_id'] = $doc_documento_id;
        return $fc_nota_credito_documento_ins;
    }

    public function fc_nota_credito_relacionada_alta_bd(bool $header, bool $ws = false){

        $fc_nota_creditos_id = $_POST['fc_nota_creditos_id'];
        $alta = array();
        foreach ($fc_nota_creditos_id as $fc_relacion_id=>$fc_nota_credito_id){
            $fc_nota_credito_relacionada_ins['fc_relacion_id'] = $fc_relacion_id;
            $fc_nota_credito_relacionada_ins['fc_nota_credito_id'] = $fc_nota_credito_id;
            $r_fc_nota_credito_relacionada = (new fc_nota_credito_relacionada(link: $this->link))->alta_registro(registro: $fc_nota_credito_relacionada_ins);
            if(errores::$error){
                return $this->retorno_error(mensaje: 'Error al dar de alta registro', data: $r_fc_nota_credito_relacionada,
                    header:  true, ws: $ws);
            }
            $alta[] = $r_fc_nota_credito_relacionada;
        }
    ...
    public function fc_relacion_alta_bd(bool $header, bool $ws = false){
        $fc_relacion_ins['fc_nota_credito_id'] = $this->registro_id;
    ...
    private function file_doc_pdf(stdClass $fc_nota_credito, string $pdf): array
    {
        $file['name'] = $fc_nota_credito->fc_nota_credito_folio.'.pdf';
        $file['tmp_name'] = $pdf;
        return $file;
    }

    private function genera_factura_pdf(stdClass $fc_nota_credito, int $fc_nota_credito_id, string $pdf): array|stdClass
    {
        $doc_documento_id = $this->doc_documento_id(fc_nota_credito: $fc_nota_credito,pdf:  $pdf);
        if(errores::$error){
            return $this->errores->error(mensaje: 'Error al al insertar documento',data:  $doc_documento_id);
        }

        $r_fc_nota_credito_documento = $this->inserta_fc_nota_credito_documento(doc_documento_id: $doc_documento_id,fc_nota_credito_id: $fc_nota_credito_id);
        if(errores::$error){
            return $this->errores->error(mensaje: 'Error al al insertar factura_doc',data:  $r_fc_nota_credito_documento);
        }
        return $r_fc_nota_credito_documento;
    }
    ...
    public function genera_pdf(bool $header, bool $ws = false){
        ...
        $fc_nota_credito = (new fc_nota_credito(link: $this->link))->registro(registro_id: $this->registro_id, retorno_obj: true);
        if(errores::$error){
            return $this->errores->error(mensaje: 'Error al obtener factura',data:  $fc_nota_credito);
        }

        $r_fc_nota_credito_documento = $this->inicializa_factura_documento(fc_nota_credito: $fc_nota_credito, fc_nota_credito_id: $this->registro_id,pdf:  $pdf);
        if(errores::$error){
            return $this->retorno_error(mensaje: 'Error al al insertar factura_doc',data:  $r_fc_nota_credito_documento, header: $header,ws:$ws);
        }

        if($header){
            $fichero = $r_fc_nota_credito_documento->registro_obj->doc_documento_ruta_absoluta;
    ...
    public function genera_xml(bool $header, bool $ws = false){
        ...
        $factura = (new fc_nota_credito(link: $this->link))->genera_xml(fc_nota_credito_id: $this->registro_id, tipo: $tipo);
        if(errores::$error){
    ...
    public function get_factura_pac(bool $header, bool $ws = false){
        $fc_nota_credito = $this->modelo->registro(registro_id: $this->registro_id);
        if(errores::$error){
            return $this->errores->error(mensaje: 'Error al obtener fc_nota_credito', data:  $fc_nota_credito);
        }
    ...
    private function inicializa_factura_documento(stdClass $fc_nota_credito, int $fc_nota_credito_id, string $pdf): array|stdClass
    {


        $r_fc_nota_credito_documento = $this->init_factura_documento(fc_nota_credito_id: $fc_nota_credito_id);
        if(errores::$error){
            return $this->errores->error(mensaje: 'Error al al eliminar registro',data:  $r_fc_nota_credito_documento);
        }

        $r_fc_nota_credito_documento = $this->genera_factura_pdf(fc_nota_credito: $fc_nota_credito, fc_nota_credito_id: $fc_nota_credito_id, pdf: $pdf);
        if(errores::$error){
            return $this->errores->error(mensaje: 'Error al al insertar factura_doc',data:  $r_fc_nota_credito_documento);
        }
        return $r_fc_nota_credito_documento;
    ...
    private function init_factura_documento(int $fc_nota_credito_id): bool|array
    {
        $filtro['fc_nota_credito.id'] = $fc_nota_credito_id;
        $filtro['doc_tipo_documento.id'] = 8;

        $existe_factura_documento = (new fc_nota_credito_documento(link: $this->link))->existe(filtro: $filtro);
        if(errores::$error){
        ...
        if($existe_factura_documento){
            $r_fc_nota_credito_documento = (new fc_nota_credito_documento(link: $this->link))->elimina_con_filtro_and(filtro: $filtro);
            if(errores::$error){
                return $this->errores->error(mensaje: 'Error al al eliminar registro',data:  $r_fc_nota_credito_documento);
            }
        }
    ...
    public function init_links(): array|string
    {
        ...
        $this->link_fc_nota_credito_nueva_partida = $link;
    ...
    private function init_modifica(bool $fecha_original, array $params = array()): array|stdClass
    {
        ...
        $sub_total = (new fc_nota_credito($this->link))->get_factura_sub_total(registro_id: $this->registro_id);
        if(errores::$error){
            return $this->errores->error(mensaje: 'Error al obtener sub_total',data:  $sub_total);
        }

        $descuento = (new fc_nota_credito($this->link))->get_factura_descuento(fc_nota_credito_id: $this->registro_id);
        if(errores::$error){
            return $this->errores->error(mensaje: 'Error al obtener descuento',data:  $descuento);
        }

        $imp_trasladados = (new fc_nota_credito($this->link))->get_factura_imp_trasladados(fc_nota_credito_id:
            $this->registro_id);
        if(errores::$error){
            return $this->errores->error(mensaje: 'Error al obtener imp_trasladados',data:  $imp_trasladados);
        }

        $imp_retenidos = (new fc_nota_credito($this->link))->get_factura_imp_retenidos(fc_nota_credito_id:
            $this->registro_id);
        if(errores::$error){
            return $this->errores->error(mensaje: 'Error al obtener imp_retenidos',data:  $imp_retenidos);
        }

        $total = (new fc_nota_credito($this->link))->get_factura_total(fc_nota_credito_id: $this->registro_id);
        if(errores::$error){
    ...
    private function inputs_partida(fc_partida_html $html, stdClass $fc_partida,
        ...
        $factura_id_disabled = $params->fc_nota_credito_id->disabled ?? true;
        $fc_nota_credito_id = $html->input_id(cols: 12,row_upd:  $fc_partida, value_vacio: false,
            disabled: $factura_id_disabled,place_holder: 'ID factura');
        if(errores::$error){
            return $this->errores->error(mensaje: 'Error al obtener sucursal_id select',data:  $fc_nota_credito_id);
        }
    ...
    private function inserta_fc_nota_credito_documento(int $doc_documento_id, int $fc_nota_credito_id): array|stdClass
    {
        $fc_nota_credito_documento_ins = $this->fc_nota_credito_documento_ins(doc_documento_id: $doc_documento_id,fc_nota_credito_id: $fc_nota_credito_id);
        if(errores::$error){
            return $this->errores->error(mensaje: 'Error al obtener fc_nota_credito_documento_ins',data:  $fc_nota_credito_documento_ins);
        }

        $r_fc_nota_credito_documento = (new fc_nota_credito_documento(link: $this->link))->alta_registro(registro: $fc_nota_credito_documento_ins);
        if(errores::$error){
            return $this->errores->error(mensaje: 'Error al al insertar factura_doc',data:  $r_fc_nota_credito_documento);
        }
        return $r_fc_nota_credito_documento;
    ...
    public function inserta_notificacion(bool $header, bool $ws = false){
        ...
        $this->link->beginTransaction();
        $notificaciones = (new fc_nota_credito(link: $this->link))->inserta_notificacion(registro_id: $this->registro_id);
        if (errores::$error) {
            $this->link->rollBack();
    ...
    public function modifica(bool $header, bool $ws = false): array|stdClass
    {
        ...
        $partidas = (new _partidas_html())->genera_partidas_html(fc_nota_credito_id: $this->registro_id,html: $this->html,link: $this->link);
        if (errores::$error) {
        ...
        $button_fc_nota_credito_relaciones =  $this->html->button_href(accion: 'relaciones', etiqueta: 'Relaciones',
            registro_id: $this->registro_id,
            seccion: 'fc_nota_credito', style: 'warning', params: array());
        if (errores::$error) {
            return $this->errores->error(mensaje: 'Error al generar link', data: $button_fc_nota_credito_relaciones);
        }

        $this->button_fc_nota_credito_relaciones = $button_fc_nota_credito_relaciones;

        $button_fc_nota_credito_timbra =  $this->html->button_href(accion: 'timbra_xml', etiqueta: 'Timbra',
            registro_id: $this->registro_id,
            seccion: 'fc_nota_credito', style: 'danger', params: array());
        if (errores::$error) {
            return $this->errores->error(mensaje: 'Error al generar link', data: $button_fc_nota_credito_relaciones);
        }

        $this->button_fc_nota_credito_timbra = $button_fc_nota_credito_timbra;

        $button_fc_nota_credito_correo =  $this->html->button_href(accion: 'correo', etiqueta: 'Correos',
            registro_id: $this->registro_id,
            seccion: 'fc_nota_credito', style: 'success', params: array());
        if (errores::$error) {
            return $this->errores->error(mensaje: 'Error al generar link', data: $button_fc_nota_credito_correo);
        }

        $this->button_fc_nota_credito_correo = $button_fc_nota_credito_correo;
    ...
    private function nueva_partida_inicializa(): array|stdClass{
        ...
        $keys_selects['fc_nota_credito_id']->id_selected = $this->registro_id;
        $keys_selects['fc_nota_credito_id']->filtro = array("fc_nota_credito.id" => $this->registro_id);
        $keys_selects['fc_nota_credito_id']->disabled = true;
        $keys_selects['fc_nota_credito_id']->cols = 12;
    ...
    private function params_button_partida(string $accion_retorno, int $fc_nota_credito_id): array
    {
        $params = array();
        $params['seccion_retorno'] = 'fc_nota_credito';
        $params['accion_retorno'] = $accion_retorno;
        $params['id_retorno'] = $fc_nota_credito_id;
        return $params;
    }
    ...
    public function partidas(bool $header, bool $ws = false): array|stdClass
    {
        ...
        $inputs = (new fc_nota_credito_html(html: $this->html_base))->genera_inputs_fc_partida(controler:$this,
            link: $this->link);
        if(errores::$error){
            return $this->retorno_error(mensaje: 'Error al inicializar inputs',data:  $inputs, header: $header,ws:$ws);
        }

        $select = $this->select_fc_nota_credito_id();
        if(errores::$error){
    ...
    public function modifica_partida(bool $header, bool $ws = false): array|stdClass
    {
        ...
        $inputs = (new fc_nota_credito_html(html: $this->html_base))->genera_inputs_fc_partida_modifica(controler:$this,
            link: $this->link);
        if(errores::$error){
            return $this->retorno_error(mensaje: 'Error al inicializar inputs',data:  $inputs, header: $header,ws:$ws);
        }
    ...
    public function modifica_partida_bd(bool $header, bool $ws = false): array|stdClass
    {
        ...
        $fc_partida_ins['fc_nota_credito_id'] = $this->registro_id;
    ...
    public function relaciones(bool $header, bool $ws = false){
        ...
        $link = $this->obj_link->link_con_id(accion: 'fc_nota_credito_relacionada_alta_bd',link:  $this->link, registro_id: $this->registro_id, seccion: $this->tabla);
        if (errores::$error) {
            $error = $this->errores->error(mensaje: 'Error al obtener link', data: $link);
            print_r($error);
            die('Error');
        ...
        $this->link_fc_nota_credito_relacionada_alta_bd = $link;

        $relaciones = (new fc_nota_credito(link: $this->link))->get_data_relaciones(fc_nota_credito_id: $this->registro_id);
        if (errores::$error) {
        ...
        $r_fc_nota_credito = (new fc_nota_credito(link: $this->link))->filtro_and(filtro: $filtro);
        if (errores::$error) {
            $error = $this->errores->error(mensaje: 'Error al obtener facturas', data: $r_fc_nota_credito);
            print_r($error);
            die('Error');
        }
        $facturas_cliente = $r_fc_nota_credito->registros;
        ...
        foreach ($relaciones as $indice=>$relacion){
            ...
            foreach ($facturas_cliente as $factura_cliente){
                ...
                foreach ($relacion['fc_nota_creditos_relacionadas'] as $fc_nota_credito_relacionada){

                    if($factura_cliente['fc_nota_credito_id'] === $fc_nota_credito_relacionada['fc_nota_credito_id']){
            ...
            $relaciones[$indice]['fc_nota_creditos'] = $facturas_cliente_;
        ...
        foreach ($relaciones as $indice=>$relacion){
            ...
            foreach ($relacion['fc_nota_creditos_relacionadas'] as $indice_fr=>$fc_nota_credito_relacionada){

                $params = $this->params_button_partida(accion_retorno: 'relaciones', fc_nota_credito_id: $this->registro_id);
                if (errores::$error) {
                    return $this->errores->error(mensaje: 'Error al generar params', data: $params);
                }

                $link_elimina_rel = $this->html->button_href(accion: 'elimina_bd', etiqueta: 'Eliminar',
                    registro_id: $fc_nota_credito_relacionada['fc_nota_credito_relacionada_id'],
                    seccion: 'fc_nota_credito_relacionada', style: 'danger',icon: 'bi bi-trash',
                ...
                $relaciones[$indice]['fc_nota_creditos_relacionadas'][$indice_fr]['elimina_bd'] = $link_elimina_rel;
        ...
        $button_fc_nota_credito_modifica =  $this->html->button_href(accion: 'modifica', etiqueta: 'Ir a Factura',
            registro_id: $this->registro_id,
            seccion: 'fc_nota_credito', style: 'warning', params: array());
        if (errores::$error) {
            return $this->errores->error(mensaje: 'Error al generar link', data: $button_fc_nota_credito_modifica);
    ...
    private function select_fc_nota_credito_id(): array|string
    {
        $select = (new fc_nota_credito_html(html: $this->html_base))->select_fc_nota_credito_id(cols:12,con_registros: true,
        ...
        $this->inputs->select->fc_nota_credito_id = $select;
    ...
    public function timbra_xml(bool $header, bool $ws = false): array|stdClass{
        ...
        $timbre = (new fc_nota_credito(link: $this->link))->timbra_xml(fc_nota_credito_id: $this->registro_id);
        if(errores::$error){
    ...
    public function verifica_cancelacion(bool $header, bool $ws = false){
        ...
        $verifica = (new timbra())->consulta_estado_sat($fc_nota_credito['org_empresa_rfc'], $fc_nota_credito['com_cliente_rfc'],
            $fc_nota_credito['fc_nota_credito_total'], $fc_nota_credito['fc_nota_credito_uuid']);
